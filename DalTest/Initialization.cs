...
private static void creatTask()
{
        ...
        s_dal!.Task.Create(new Task(0,0,"Signing a contract", "The legal process of adding the formal signature to a contract, signifying agreement and commitment", false,start.AddDays(s_rand.Next(rangStart)),null,null,null,null,null, "Formally signed contract indicating mutual agreement and commitment.", (DO.Expirience)0));
        ...
        {
            s_dal!.Task.Create(new Task(0, 0, TaskNameForHandyman[i], TaskDescriptionForHandyman[i], false, start.AddDays(s_rand.Next(rangStart)), null, null, null, null, null, TaskResultForHandyman[i], (DO.Expirience)1));
        }
        for (int i = 0; i < TaskNamePainter.Length; i++)
        {
            s_dal!.Task.Create(new Task(0, 0, TaskNamePainter[i], TaskDescriptionForPainter[i], false, start.AddDays(s_rand.Next(rangStart)), null, null, null, null, null, TaskResultForPainter[i], (DO.Expirience)2));
        }
        for (int i = 0; i < TaskNameArchitect.Length; i++)
        {
            s_dal!.Task.Create(new Task(0, 0, TaskNameArchitect[i], TaskDescriptionForArchitect[i], false, start.AddDays(s_rand.Next(rangStart)), null, null, null, null, null, TaskResultForArchitect[i], (DO.Expirience)3));
        }
        for (int i = 0; i < TaskNameInteriorDesigner.Length; i++)
        {
            s_dal!.Task.Create(new Task(0, 0, TaskNameInteriorDesigner[i], TaskDescriptionForInteriorDesigner[i], false, start.AddDays(s_rand.Next(rangStart)), null, null, null, null, null, TaskResultForInteriorDesigner[i], (DO.Expirience)4));
        }
